...
- **Liveness** comes from the kernel: each file's pid is asked when its process started
  (`proc_pidinfo`, one syscall of about a microsecond), every sweep. A session waiting for input is silent for hours and alive; a session in a
...
2. **Screen.** The daemon writes `~/.hands/status.json` every heartbeat with its pid,
   ...
   the kernel (`proc_pidinfo`, about a microsecond), and it is the rule the session
   sweep uses, from `hands.sessions.processes`. A pid that a later process took, after
   a crash or a reboot, reads as down rather than as not responding. So does one held
   by another user's process, since hands' processes all run as the user.
