...
async def test_claude_answering_under_a_new_id_carries_when_claude_code_wrote_it(tmp_path: Path, stamp: str | None, written: Stamp | None) -> None:
    """On the clock Claude Code stamps a status with, so an idle it set after the answer can be told from one before."""
    transcript = tmp_path / "t.jsonl"
    answer = WRITING if stamp is None else WRITING.replace('"type":"assistant"', f'"type":"assistant","timestamp":{stamp}')
    transcript.write_text(lines(ASKED, WRITING, ASKED.replace('"p1"', '"p2"'), answer))
    assert (await Tails(Registry([member(transcript)])).catch_up())[-1] == Continued(SID, was=PromptId("p1"), now=PromptId("p2"), written=written, at=7.0)
...
async def test_a_record_whose_time_cannot_be_read_is_said_read_as_having_none_and_still_told(tmp_path: Path, stamp: str) -> None:
    ...
    transcript = tmp_path / "t.jsonl"
    transcript.write_text(lines(ASKED, WRITING, ASKED.replace('"p1"', '"p2"'), WRITING.replace('"type":"assistant"', f'"type":"assistant","timestamp":{stamp}')))
    errors: list[str] = []
    sink = logger.add(lambda message: errors.append(message.record["message"]), level="ERROR", filter="hands")
    try:
        tails = Tails(Registry([member(transcript)]))
        assert (await tails.catch_up())[-1] == Continued(SID, was=PromptId("p1"), now=PromptId("p2"), written=None, at=7.0)
    finally:
        logger.remove(sink)
    assert len(errors) == 1 and "has a time that cannot be read" in errors[0]
    telling = await tails.tell(SID, None, None)
    assert telling is not None and telling.turn.steps == (Said(None, "# Rivers"),)
...
async def test_a_turn_that_goes_on_under_a_queued_prompt_is_heard_to_after_the_interrupt_that_flushed_it(tmp_path: Path) -> None:
    ...
    tails = Tails(Registry([member(transcript)]))
    assert await tails.catch_up() == [Taken(SID, PromptId("p1")), Taken(SID, PromptId("p2")), Interrupted(SID, PromptId("p2"), at=7.0), Continued(SID, was=PromptId("p1"), now=PromptId("p2"), written=None, at=7.0)]
...
async def test_a_queued_command_taken_in_mid_turn_is_heard_from_the_results_claude_answers(tmp_path: Path) -> None:
    ...
    assert await Tails(Registry([member(transcript)])).catch_up() == [Taken(SID, PromptId("p1")), Taken(SID, PromptId("p2")), Continued(SID, was=PromptId("p1"), now=PromptId("p2"), written=None, at=7.0)]
