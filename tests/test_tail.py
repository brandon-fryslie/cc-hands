...
async def test_a_turn_cut_off_while_claude_wrote_is_heard_as_interrupted_and_told_with_what_it_had_said(tmp_path: Path) -> None:
    ...
    tails = Tails(Registry([member(transcript)]))
    assert await tails.catch_up() == [Taken(SID, PromptId("p1"), None, at=7.0), Interrupted(SID, PromptId("p1"), at=7.0)]
    telling = await tails.tell(SID, None, None)
...
async def test_a_prompt_that_only_mentions_an_interrupt_is_a_prompt(tmp_path: Path) -> None:
    ...
    tails = Tails(Registry([member(transcript)]))
    assert await tails.catch_up() == [Taken(SID, PromptId("p1"), None, at=7.0)]
    telling = await tails.tell(SID, None, None)
    assert telling is not None and telling.turn.opening == Asked(None, "Why did I see [Request interrupted by user] there?")


async def test_a_prompt_is_taken_when_claude_code_wrote_its_record_on_the_clock_it_stamps_a_status_with(tmp_path: Path) -> None:
    transcript = tmp_path / "t.jsonl"
    transcript.write_text(lines(ASKED.replace('"promptId"', '"timestamp":"2026-09-24T17:48:23.455Z","promptId"')))
    assert await Tails(Registry([member(transcript)])).catch_up() == [Taken(SID, PromptId("p1"), Stamp(1790272103455), 7.0)]


async def test_a_record_whose_time_cannot_be_read_is_said_and_taken_as_nothing(tmp_path: Path) -> None:
    transcript = tmp_path / "t.jsonl"
    transcript.write_text(lines(ASKED.replace('"promptId"', '"timestamp":"yesterday","promptId"')))
    errors: list[str] = []
    sink = logger.add(lambda message: errors.append(message.record["message"]), level="ERROR", filter="hands")
    try:
        ...
        logger.remove(sink)
    assert len(errors) == 1 and "'yesterday' is not a time" in errors[0]
...
async def test_an_interrupt_whose_record_names_no_turn_is_said_and_ends_nothing(tmp_path: Path) -> None:
    ...
    sink = logger.add(lambda message: errors.append(message.record["message"]), level="ERROR", filter="hands")
    try:
        assert await Tails(Registry([member(transcript)])).catch_up() == [Taken(SID, PromptId("p1"), None, at=7.0)]
    finally:
        logger.remove(sink)
...
async def test_the_older_record_of_an_interrupt_written_as_a_plain_string_is_one_too(tmp_path: Path) -> None:
    ...
    assert await Tails(Registry([member(transcript)])).catch_up() == [Taken(SID, PromptId("p1"), None, at=7.0), Interrupted(SID, PromptId("p1"), at=7.0)]
...
async def test_a_turn_that_goes_on_under_a_queued_prompt_is_heard_to_after_the_interrupt_that_flushed_it(tmp_path: Path) -> None:
    ...
    tails = Tails(Registry([member(transcript)]))
    assert await tails.catch_up() == [Taken(SID, PromptId("p1"), None, at=7.0), Taken(SID, PromptId("p2"), None, at=7.0), Interrupted(SID, PromptId("p2"), at=7.0), Continued(SID, was=PromptId("p1"), now=PromptId("p2"))]
...
async def test_a_queued_command_taken_in_mid_turn_is_heard_from_the_results_claude_answers(tmp_path: Path) -> None:
    ...
    assert await Tails(Registry([member(transcript)])).catch_up() == [Taken(SID, PromptId("p1"), None, at=7.0), Taken(SID, PromptId("p2"), None, at=7.0), Continued(SID, was=PromptId("p1"), now=PromptId("p2"))]
