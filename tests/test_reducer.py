...
SESSION_EVENTS: list[SessionEvent] = [
    Prompted(ONE.id, at=5.0, mode=None),
...
def holding(state: SessionState) -> Registry:
    return registry(Session(ONE, state, mode=None))
...
@pytest.mark.parametrize(
    ("event", "after"),
    [
        (Prompted(ONE.id, at=5.0, mode=None), Working(since=5.0)),
...
@pytest.mark.parametrize("event", [Prompted(ONE.id, at=5.0, mode=None), Ended(ONE.id, "prompt_input_exit"), Joined(ONE, "startup")])
...
@pytest.mark.parametrize(
    "event", [Prompted(ONE.id, at=5.0, mode=None), Ended(ONE.id, "prompt_input_exit"), *(Joined(ONE, source) for source in ("startup", "resume", "clear"))]
)
...
def test_a_prompt_inside_a_running_turn_leaves_the_mark_where_that_turn_began(before: SessionState) -> None:
    ...
    """
    assert Snapshot(ONE.id, ONE.cwd) not in reduce(holding(before), Prompted(ONE.id, at=5.0, mode=None))[1]
...
def test_a_prompt_marks_where_the_repository_stands_before_the_turn_can_change_it() -> None:
    ...
    assert reduce(holding(Idle()), Prompted(ONE.id, at=5.0, mode=None))[1] == [Snapshot(ONE.id, ONE.cwd)]
...
def test_a_tick_moves_only_sessions_waiting_on_a_deadline() -> None:
    before = registry(Session(ONE, WAITING, mode=None), Session(TWO, Working(since=0.0), mode=None))
    after, _ = reduce(before, Tick(at=100.0))
    assert after == registry(Session(ONE, Working(since=100.0), mode=None), Session(TWO, Working(since=0.0), mode=None))
...
def test_a_compacted_session_keeps_its_state_and_takes_the_new_membership(before: SessionState) -> None:
    ...
    assert reduce(holding(before), Joined(moved, "compact")) == (registry(Session(moved, before, mode=None)), [])
...
def test_an_event_moves_only_its_own_session() -> None:
    ...
    after, _ = reduce(before, Prompted(TWO.id, at=2.0, mode=None))
    assert after == registry(Session(ONE, Idle(), mode=None), Session(TWO, Working(since=2.0), mode=None))


def test_live_is_every_session_that_has_not_ended() -> None:
    both = registry(Session(ONE, Blocked(on=BASH, request=RequestId("r"), deadline=9.0, warned=False), mode=None), Session(TWO, Gone(), mode=None))
    assert both.live() == [Session(ONE, Blocked(on=BASH, request=RequestId("r"), deadline=9.0, warned=False), mode=None)]
...
def test_a_nudged_session_prompted_again_marks_the_repository_its_turn_starts_from() -> None:
    assert reduce(holding(Idle(nudged=True)), Prompted(ONE.id, at=5.0, mode=None)) == (holding(Working(since=5.0)), [Snapshot(ONE.id, ONE.cwd)])
...
@pytest.mark.parametrize("opened", [Prompted(ONE.id, at=5.0, mode=None), Joined(ONE, "compact"), Joined(ONE, "resume")])
...
REPORTING: list[SessionEvent] = [
    Prompted(ONE.id, at=5.0, mode="plan"),
...
def moded(state: SessionState, mode: Mode | None) -> Registry:
    return registry(Session(ONE, state, mode=mode))
...
def test_a_mode_reported_again_unchanged_is_not_noted() -> None:
    _, effects = reduce(moded(Idle(), "plan"), Prompted(ONE.id, at=5.0, mode="plan"))
    assert not [effect for effect in effects if isinstance(effect, Note)]
...
def test_a_mode_hands_does_not_know_is_noted_like_any_other() -> None:
    _, effects = reduce(moded(Idle(), "default"), Prompted(ONE.id, at=5.0, mode=UnknownMode("ultraplan")))
