...
@pytest.mark.parametrize(
    ("event", "after"),
    [
        (Stopped(ONE.id, None, mode=None, prompt=TURN, again=False), Idle()),
        (
            PermissionRequested(ONE.id, at=5.0, request=RequestId("r1"), on=BASH, mode=None),
...
def test_a_session_prompted_again_and_left_again_is_nudged_again(opened: Event) -> None:
    heard: list[Effect] = []
    state = holding(Idle())
    for event in [Waited(ONE.id), Waited(ONE.id), opened, Stopped(ONE.id, None, mode=None, prompt=TURN, again=False), Waited(ONE.id), Waited(ONE.id)]:
        state, effects = reduce(state, event)
        heard += [effect for effect in effects if isinstance(effect, Speak)]
...
def test_the_stop_of_a_turn_over_before_the_next_prompt_ends_nothing(state: SessionState) -> None:
    ...
    after, effects = reduce(in_turn(state, turn=NEXT), Stopped(ONE.id, "Done.", mode="plan", prompt=TURN, again=False))
...
def test_a_stop_at_the_prompt_tells_the_turn_it_names(turn: PromptId | None, again: bool) -> None:
...
def test_the_stop_of_a_turn_told_at_the_prompt_ends_nothing_and_keeps_the_idle_period_it_lands_in() -> None:
    """An interrupted turn's Stop can land after it was told: told again, it would be heard twice. Claude Code sends no
    idle_prompt for that period, so the nudge hands is timing for it still comes."""
    waiting = Idle(due=70.0)
    assert reduce(in_turn(waiting), STOP) == (in_turn(waiting), [])
...
def test_a_prompt_or_record_naming_another_turn_while_one_runs_ends_nothing(state: SessionState, event: Event) -> None:
    ...
    assert reduce(after, Stopped(ONE.id, "Next.", mode=None, prompt=NEXT, again=False))[0].sessions[ONE.id].state == Idle()
